...
def parse_privmsg(conn, nick, user, host, chan, irc_raw, has_permission):
    if not has_permission('floodexempt', notice=False):
        ...
        if len(message) > 0:
            ...
            if config:
                ...
                check_flood(conn, chan, nick, user, host, config)
                check_repeat(conn, chan, nick, user, host, config)
                check_caps(conn, chan, nick, user, host, message, config)
...
def on_join(conn, chan, nick):
    ...
    if config:
        ...
        if nick == conn.nick:
            setup_user(conn, chan, lines, repeat)
...
def check_flood(conn, chan, nick, ident, host, config):
    opchan = conn.config.get("chanprotect", {}).get(chan.lower(), {})['opchan']
    ...
    if len(user_times) == lines:
        ...
        if dt.total_seconds() < secs:
            ...
            action(conn, opchan, nick, ident, host, chan, count, lines=lines, secs=secs)

def check_repeat(conn, chan, nick, ident, host, config):
    opchan = conn.config.get("chanprotect", {}).get(chan.lower(), {})['opchan']
    max = config['repeat']
    lnick = nick.lower(); lchan = chan.lower()
    ...
    if len(user_messages) == max:
        if len(set(user_messages)) == 1:
            add_strike(conn, chan, nick)
            count = strikes[conn.name][lchan][lnick]
            action(conn, opchan, nick, ident, host, chan, count, max=max)

def check_caps(conn, chan, nick, ident, host, message, config):
    config = conn.config.get("chanprotect", {}).get(chan.lower(), {})
    upper_count = 0
    ...
    if percent >= config['capratio'] and len(message) >= config['caplength']:
        ...
        lnick = nick.lower(); lchan = chan.lower()
        count = strikes[conn.name][lchan][lnick]
        action(conn, config['opchan'], nick, ident, host, chan, count, percent=percent)

@hook.command("warn", permissions=["op"])
def warn(conn, nick, user, host, text, notice):
    ...
    try:
        ...
        add_strike(conn, chan, nick)
        count = strikes[conn.name][lchan][lnick]
        action(conn, opchan, nick, user, host, chan, count, manual=True, reason=reason, op=op, notice=notice)
    except:
        return notice(".warn <nick> <#channel> <reason>")

def action(conn, opchan, nick, ident, host, chan, count, **kwargs):
    ...
    op_notice = "PRIVMSG {} :\x02CHANPROTECT\x02: User {} triggered {} ({}/3): {}".format(opchan, user, action, count, reason)
    if kwargs.get('manual', False):
        op_notice = "{} (op: {})".format(op_notice, op)
    conn.send(op_notice)
...
def getstrikes(conn, text, notice):
    try:
        text = text.split()
        nick = text[0]
        chan = text[1]
...
def clearstrikes(conn, nick, text, notice):
    ...
    try:
        ...
        conn.send(out)
        out = "PRIVMSG {} :\x02CHANPROTECT\x02: {} cleared strikes for {} in {}.".format(opchan, op, nick, chan)
        conn.send(out)
    except:
